...
export default function MealCreationPage() {
  ...
  const onSubmit = async (data: MealFormData) => {
    const mealData = { ...data, ingredients, steps, images };

    try {
      const response = await fetch(
        meal
          ? `${BASE_URL}/api/meal/v1/update-admin-meal/${meal._id}` // Edit API
          : `${BASE_URL}/api/meal/v1/create-admin-meal`, // Create API
        {
  ...
  return (
    <>
      <Card className="w-full max-w-2xl mx-auto shadow-xl rounded-2xl bg-white p-6">
        <CardHeader>
